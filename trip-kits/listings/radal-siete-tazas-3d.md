# SKU: radal-siete-tazas-3d — US$12.90 (kit gen-2, ESPAÑOL)

Segundo kit en español: apunta a la demanda GSC medida en guias.viajesypanoramas.cl
("velo de la novia" 62 impresiones, posición ~9 — ranking bueno, sin producto que la
capture). Contenido 100% condensado de la guía ES `radal-siete-tazas-curico.html`.
Cero datos inventados. SIN sección de POIs bonus: la cobertura del catálogo Panoramas
en Molina/Curicó es boilerplate autogenerado (verificado 2026-08-01 — decisión y
evidencia en `kits.config.mjs`, entry del kit).
La descripción de marketplace va en español a propósito (kit ES); la sección se llama
"Etsy description" solo para que `gen-product-specs.mjs` la parsee como los kits gen-1.

## Título (Payhip/Gumroad)
Radal Siete Tazas y Velo de la Novia: 3 días en el Maule — Itinerario imprimible (PDF)

## Tags / keywords
radal siete tazas / velo de la novia / curicó / molina / ruta del vino curicó /
salto la leona / termas panimávida / itinerario chile / guía de viaje pdf /
descarga digital / imprimible viaje / maule / planificador viaje

## Etsy description
Radal Siete Tazas ordenado día a día: 3 días entre los pozones turquesa del río
Claro, las cascadas Salto La Leona y Velo de la Novia, la Ruta del Vino de Curicó
en el valle de Lontué y el cierre en las termas de Panimávida — con la logística
real de cómo llegar y subir al parque desde Curicó o Molina.

QUÉ INCLUYE (PDF A4, descarga instantánea):
• Plan día a día: llegada a Curicó → día completo en el parque (pozones del río
  Claro, Salto La Leona y Velo de la Novia) → Ruta del Vino de Curicó y cierre
  termal en Panimávida o Quinamávida
• Mapa esquemático de la ruta de 3 días
• Tabla de presupuesto 2026 en CLP: entrada al parque, tour de degustación en
  viña, almuerzo con maridaje, circuito termal, alojamiento en Curicó o Molina
• Checklist pre-viaje: entrada comprada online en pasesparques.cl (no hay venta
  en el lugar), transporte Molina → parque, reserva de viña en vendimia
• FAQ: compra anticipada de entradas, cuántos días se necesitan, si se puede
  visitar en un día desde Santiago

POR QUÉ CONFIAR: lo compila el equipo detrás de guias.viajesypanoramas.cl, sitio
gratuito de guías de Chile construido sobre un catálogo de más de 25.000 lugares
reales. Este kit destila esa guía en una ruta que puedes seguir sin conexión.

Datos de lugares derivados de OpenStreetMap, (c) colaboradores de OpenStreetMap,
disponibles bajo la Open Database License (ODbL).

DESCARGA INSTANTÁNEA — no se envía ningún producto físico. Imprímelo o llévalo en
el teléfono. Solo para uso personal.

## Gumroad (espejo opcional, comisión 10%)
- Product name: Radal Siete Tazas y Velo de la Novia — 3 días en el Maule (PDF)
- Permalink (EXACTO): radal-siete-tazas-3d
- Price: $12.90
- Estado: PENDIENTE — producto aún no creado (2026-08-01). Sin URL hasta que exista.

## Payhip (tienda principal, comisión 5%)
- Nombre de producto: Radal Siete Tazas y Velo de la Novia: 3 días en el Maule — Itinerario imprimible (PDF)
- Permalink Payhip: PENDIENTE — el código /b/<CODE> lo genera Payhip al crear el
  producto (no inventarlo). Pegarlo luego en `PAYHIP_URLS['radal-siete-tazas-3d']`
  en `trip-kits/kits.config.mjs`.
- Precio: $12.90
- Descripción: la de arriba (quitar la línea DESCARGA INSTANTÁNEA y dejar
  "Descarga instantánea en PDF. Solo uso personal.").

## Archivos del producto
- PDF: `trip-kits/dist/radal-siete-tazas-3d-es.pdf` (10 páginas A4, 1.021.521 bytes ~998 KB)
- Mockups listing: `trip-kits/mockups-out/radal-siete-tazas-3d-main.png` (2000x2000)
  + `radal-siete-tazas-3d-preview-1..3.png` (portada, día 1, presupuesto)
- (dist/ y mockups-out/ están gitignored — regenerar con `npm run all` si hace falta)

## Cableado final (cuando el producto exista)
1. Pegar la URL Payhip en `PAYHIP_URLS['radal-siete-tazas-3d']` (kits.config.mjs).
2. Agregar `'radal-siete-tazas-3d'` a `READY_KITS` (inject-kit-cta.mjs).
3. Correr `node inject-kit-cta.mjs` → inyecta el CTA solo en la guía ES
   `radal-siete-tazas-curico.html` (antes de `.promo`, UTM `utm_campaign=radal-siete-tazas-3d`).
   La guía ya está mapeada en el `MAP` del inyector — no hay que tocar el MAP.
4. Re-correr los 5 gates (check-html, check-links, check-affiliate-ids,
   check-hreflang, check-sitemap) y pushear.
