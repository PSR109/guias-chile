# SKU: termas-del-sur-4d — US$12.90 (kit gen-2, ESPAÑOL)

Primer kit en español: apunta a la demanda GSC medida en guias.viajesypanoramas.cl
("termas de chillán" ~79 impresiones/3 semanas, posición ~45 — query caliente sin
producto que la capture). Contenido condensado de la guía ES `termas-de-chillan.html`
+ catálogo Panoramas (POIs con `descripcion_es`). Cero datos inventados.
La descripción de marketplace va en español a propósito (kit ES); la sección se llama
"Etsy description" solo para que `gen-product-specs.mjs` la parsee como los kits gen-1.

## Título (Payhip/Gumroad)
Termas del Sur: 4 días en las Termas de Chillán — Itinerario imprimible (PDF)

## Tags / keywords
termas de chillán / termas chile / nevados de chillán / ñuble / aguas termales /
itinerario chile / guía de viaje pdf / esquí chile / reserva ñuble / descarga digital /
imprimible viaje / sur de chile / planificador viaje

## Etsy description
Las Termas de Chillán ordenadas día a día: 4 días entre las aguas termales volcánicas
de los Nevados de Chillán, la nieve del centro de esquí más grande de Chile (en
temporada), el trekking de la Reserva Nacional Ñuble y la gastronomía de Chillán —
con la logística real de cómo llegar y subir al Valle Las Trancas.

QUÉ INCLUYE (PDF A4, descarga instantánea):
• Plan día a día: llegada y subida al Valle Las Trancas → día completo de aguas
  termales → nieve en invierno o trekking en la Reserva Ñuble → mercado, longaniza
  y murales de Chillán antes de volver
• Mapa esquemático de la ruta de 4 días
• Tabla de presupuesto 2026 en CLP: pase de esquí, entrada de día a las piscinas
  termales, Reserva Ñuble, cabañas en Las Trancas, mercado
• Checklist pre-viaje: reserva de entrada a las termas (aforo limitado en temporada
  alta), transporte Chillán → Las Trancas, estado de andariveles
• 8 lugares bonus de nuestro catálogo de más de 25.000 lugares en Chile
• FAQ: mejor época para esquiar, entrada sin alojarse en el hotel, cuántos días dedicar

POR QUÉ CONFIAR: lo compila el equipo detrás de guias.viajesypanoramas.cl, sitio
gratuito de guías de Chile construido sobre un catálogo de más de 25.000 lugares
reales. Este kit destila esa guía en una ruta que puedes seguir sin conexión.

Datos de lugares derivados de OpenStreetMap, (c) colaboradores de OpenStreetMap,
disponibles bajo la Open Database License (ODbL).

DESCARGA INSTANTÁNEA — no se envía ningún producto físico. Imprímelo o llévalo en
el teléfono. Solo para uso personal.

## Gumroad (espejo opcional, comisión 10%)
- Product name: Termas del Sur — 4 días en las Termas de Chillán (PDF)
- Permalink (EXACTO): termas-del-sur-4d  →  https://payhip.com/b/XDjCS
- URL Gumroad espejo (verificada 200 anónimo 2026-08-01): https://gumroad.com/l/termas-del-sur-4d
- Price: $12.90

## Payhip (tienda principal, comisión 5%)
- Nombre de producto: Termas del Sur: 4 días en las Termas de Chillán — Itinerario imprimible (PDF)
- Permalink Payhip: https://payhip.com/b/XDjCS (creado y verificado 2026-08-01 —
  "Product Added" confirmado, PDF adjunto, US$12.90)
- Precio: $12.90
- Descripción: la de arriba (quitar la línea DESCARGA INSTANTÁNEA y dejar
  "Descarga instantánea en PDF. Solo uso personal.").

## Archivos del producto
- PDF: `trip-kits/dist/termas-del-sur-4d-es.pdf` (13 páginas A4, ~567 KB)
- Mockups listing: `trip-kits/mockups-out/termas-del-sur-4d-main.png` (2000x2000)
  + `termas-del-sur-4d-preview-1..3.png` (portada, día 1, presupuesto)
- (dist/ y mockups-out/ están gitignored — regenerar con `npm run all` si hace falta)
